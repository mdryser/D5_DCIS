...
# Start-stop in x-axis direction
ind<-which(dat.pos$Slide==u[4])
max.4<-max((dat.pos$x.pos[ind]))
min.4<-min((dat.pos$x.pos[ind]))
# Z-axis position
z.4<-dat.pos$z.pos[ind[1]]*f


# Start-stop in x-axis direction
ind<-which(dat.pos$Slide==u[5])
max.5<-max((dat.pos$x.pos[ind]))
min.5<-min((dat.pos$x.pos[ind]))
# Z-axis position
z.5<-dat.pos$z.pos[ind[1]]*f
...
ggplot(dat.pos, aes(x=x.pos+f*z.pos, y=y.pos, label=Spot))+
  annotate("rect", xmin = z.1-max.x, xmax = z.1+max.x, ymin = -max.y, ymax = max.y,
           alpha = .3,fill = pal1[3]) +
  annotate("text", x = z.1, y = f3*max.y, label = u[1]) +
  annotate("rect", xmin = z.2-max.x, xmax = z.2+max.x, ymin = -max.y, ymax = max.y,
           alpha = .3,fill =  pal1[3])+
  annotate("text", x = z.2, y = f3*max.y, label = u[2]) +
  annotate("rect", xmin = z.3-max.x, xmax = z.3+max.x, ymin = -max.y, ymax = max.y,
           alpha = .3,fill =  pal1[3])+
  annotate("text", x = z.3, y = f3*max.y, label = u[3]) +
  annotate("rect", xmin = z.4-max.x, xmax = z.4+max.x, ymin = -max.y, ymax = max.y,
           alpha = .3,fill =  pal1[3])+
  annotate("text", x = z.4, y = f3*max.y, label = u[4]) +
  annotate("rect", xmin = z.5-max.x, xmax = z.5+max.x, ymin = -max.y, ymax = max.y,
           alpha = .3,fill =  pal1[3])+
  annotate("text", x = z.5, y = f3*max.y, label = u[5]) +
  ...
  coord_fixed(ratio=1)+  
  scale_x_continuous("Long axis (cm)", breaks=c( z.1,z.2,z.3, z.4, z.5), labels=c( c(z.1,z.2,z.3, z.4, z.5)/f))+
  ylab("y-axis (cm)")+
  ...
  annotate(geom="text", x=bar.x+.2, y=bar.y+.1, label="5mm",
           color="black", size=2)+
  ggtitle(tum)
