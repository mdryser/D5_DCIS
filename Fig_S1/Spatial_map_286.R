...
ggplot(dat.pos, aes(x=x.pos+f*z.pos, y=y.pos, label=Spot))+
  annotate("rect", xmin = z.1-max.x, xmax = z.1+max.x, ymin = -max.y, ymax = max.y,
           alpha = .3,fill = pal1[3]) +
  annotate("text", x = z.1, y = f3*max.y, label = u[1], size=2.5) +
  annotate("rect", xmin = z.2-max.x, xmax = z.2+max.x, ymin = -max.y, ymax = max.y,
           alpha = .3,fill =  pal1[3])+
  annotate("text", x = z.2, y = f3*max.y, label = u[2], size=2.5) +
  annotate("rect", xmin = z.3-max.x, xmax = z.3+max.x, ymin = -max.y, ymax = max.y,
           alpha = .3,fill =  pal1[3])+
  annotate("text", x = z.3, y = f3*max.y, label = u[3], size=2.5) +
  annotate("rect", xmin = z.4-max.x, xmax = z.4+max.x, ymin = -max.y, ymax = max.y,
           alpha = .3,fill =  pal1[3])+
  annotate("text", x = z.4, y = f3*max.y, label = u[4], size=2.5) +
  ...
  coord_fixed(ratio=1)+  
  scale_x_continuous("Long axis (cm)", breaks=c( z.1,z.2,z.3, z.4), labels=c( c(z.1,z.2,z.3, z.4)/f))+
  ylab("y-axis (cm)")+
  ...
  annotate(geom="text", x=bar.x+.2, y=bar.y+.15, label="5mm",
           color="black", size=2)
