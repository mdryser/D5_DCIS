#-----------------------------------------------
#-----------------------------------------------
#-- Mutation map for DCIS-91
#-- Use for paper Figure 4B
...
max.y<-max(abs(dat.pos$y.pos))*1.2
...
ggplot(dat.pos, aes(x=x.pos+f*z.pos, y=y.pos))+
  ...
  geom_point(aes(colour=factor(mut)), size=2 )  +
  ...
  ylab("") + 
  geom_segment(aes(x=-0.4, y=-.9, xend=.1, yend=-.9))+
  annotate(geom="text", x=-.2, y=-.8, label="5mm",
           color="black", size=2)

#-- Save the Figure 4B to PDF

ggsave("Figure_4B.pdf",
       width=20,
       height=10,
       units="cm")
